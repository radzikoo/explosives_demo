...
func _physics_process(delta): # Most things happen here.
	$UserInterface/HealthBar.value = health
	
	if direction != Vector3.ZERO:
		if $Rotator/RayCast3D.is_colliding() and not $Rotator/RayCast3D2.is_colliding():
			if $Rotator/RayCast3D.get_collider().is_in_group("Terrain"):
				if velocity.y == 0:
...
func handle_movement(delta, input_dir):
	direction = input_dir.rotated(-HEAD.rotation.y)
	direction = Vector3(direction.x, 0, direction.y)
	$Rotator/RayCast3D.target_position = direction/1.1
	$Rotator/RayCast3D2.target_position = direction/1.1
	move_and_slide()
...
func handle_head_rotation():
	if invert_camera_x_axis:
		...
		#ROTATOR.rotation_degrees.y -= mouseInput.x * mouse_sensitivity * -1
	else:
		HEAD.rotation_degrees.y -= mouseInput.x * mouse_sensitivity
		#ROTATOR.rotation_degrees.y -= mouseInput.x * mouse_sensitivity
